...
public class FunctionEmployee {
    ...
    public void updateDataEmployee() {
        ...
        try {
            Statement statement = jdbcData.getStatement();
            ResultSet getEmployeeByCode = statement.executeQuery("SELECT emp.*, dep.department_name FROM employee emp JOIN department dep " +
                    "ON emp.department_id = dep.id WHERE employee_code = '" + employeeCode + "'");

            out.printf("%-3s %-15s %-20s %-15s %-10s %-3s %-15s %-25s %-15s %-15s %-25s %-25s \n", "No.",
                    "Employee Code", "Full Name", "Position", "Gender", "Age", "Phone", "Email", "Salary",
                    "Tax", "Hire Date", "Department Name");
            ...
            while (getEmployeeByCode.next()) {
                ...
                String departmentName = getEmployeeByCode.getString("department_name");

                out.printf("%-3s %-15s %-20s %-15s %-10s %-3s %-15s %-25s %-15s %-15s %-25s %-25s \n", number,
                        employeeCode, fullName, position, gender, age, phone, email, formatDouble(salary),
                        formatDouble(tax), hireDate, departmentName);
            }
        } catch (ClassNotFoundException e) {
            e.printStackTrace();
        } catch (SQLException e) {
        ...
        while (true) {
            ...
            out.print("Enter Infomation Want To Update: ");
            ...
            if (op == 1) {
                out.print("Enter Full Name Update: ");
                String fullName = scanner.nextLine();
                ...
                if (positionCheck == 1) {
                    positon = Position.MANAGER.name();
                    salary = PositionSalary.MANAGER_SALARY;
                    tax = calculateTax(salary);
                } else if (positionCheck == 2) {
                    positon = Position.EMPLOYEE1.name();
                    salary = PositionSalary.EMPLOYEE1_SALARY;
                    tax = calculateTax(salary);
                } else {
                    positon = Position.EMPLOYEE2.name();
                    salary = PositionSalary.EMPLOYEE2_SALARY;
                    tax = calculateTax(salary);
                }
            ...
            out.print("Would you like to update this infomation? (Y/N): ");
